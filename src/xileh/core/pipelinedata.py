...
class xData:
    ...
    def __setitem__(self, name, value):
        ...
        assert isinstance(value, xData), (
            "You can only assign xData containers to a xData container"
        )
    ...
    def __repr__(self):
        ...
        s = f"xData object at {hex(id(self))} - with size {self.__sizeof__()}\n"
    ...
    def meta(self, mdict):
        ...
        for k, v in mdict.items():
            if "shape" in dir(v) and "shape" in dir(self._data):
                if not any(
                    [v.shape[i] == self._data.shape[i] for i in range(len(v.shape))]  # type: ignore
                ):
                    raise ValueError(
                        "Please provide meta data with shape property aligned"
                        " to at least one dimension of the data within this"
                        f" objects data: self.data.shape={self._data.shape}"  # type: ignore
    ...
    def check_header(self, key, missing=""):
        return self._check_dict(self.header, key, dname="header", missing=missing)

    def check_meta(self, key, missing=""):
        return self._check_dict(self.meta, key, dname="meta", missing=missing)

    def _find_by_name(self, name, find_parent=False, parent=None):
        """Recursively traverse the data container looking for a (sub)
        ...
        if self.header["name"] == name:
            return parent if find_parent else self
        ...
        if isinstance(self.data, list):
            for pd in [p for p in self.data if isinstance(p, xData)]:
                data = pd._find_by_name(name, find_parent=find_parent, parent=self)
                if data is not None:
                    return data  # early stopping

        return None

    def get_by_name(
        self, name, create_if_missing=False, find_parent=False, parent=None
    ) -> xData:
        ...
        create_if_missing : bool
            ...
            and append to the self.data if it is a list, else raise a
            ValueError. Has no effect when find_parent is True.
        find_parent : bool
            if true return the parent container instead
        parent : xileh.xData
            a container to store the parent to be returned if find_parent is
        ...
        -------
            data : xData
                A data container with the given name.

        Raises
        ------
        KeyError
            If no container with the given name is found and it is not created
            (create_if_missing is False or find_parent is True).
        """

        data = self._find_by_name(name, find_parent=find_parent, parent=parent)
        if data is not None:
            return data

        if create_if_missing and not find_parent:
            if not isinstance(self.data, list):
                raise ValueError("Cannot create for type(self.data) != list container")
            data = xData(None, name=name)
            self.data.append(data)
            return data

        raise KeyError(f"No container with {name=} found")
    ...
    def add(
        ...
        if not name:
            raise ValueError("A non-empty 'name' is required when adding a container")
    ...
    def delete_by_name(self, name):
        ...
        # make sure it is there (raises KeyError if not)
        ...
        if parent.data == trg_c:
            ...
            parent._data = CheckedList(
                [
                    ...
                    if isinstance(
                        c, xData
                    )  # note only containers can be deleted by this approach, no need to check anything else      # noqa
                    and c != trg_c
                ],
                parent,
            )
            ...
            raise ValueError(
                f"Parent of target with {name=} has an unknown"
    ...
    def _to_dict(self):
        ...
        if isinstance(self.data, list):
            ddata["data"] = [
                v._to_dict() if isinstance(v, xData) else v for v in self.data
            ]
        else:
    ...
    def rename(self, from_name, to_name):
        ...
        # a top level container has no parent -> nothing to clean up
        try:
    ...
    def copy(self):
        ...
        if isinstance(self.data, list):
            ndata.data = [
                v.copy() if isinstance(v, xData) else deepcopy(v) for v in self.data
            ]
        else:
...
def from_dict(d):
    ...
    assert d[type_key] == "xPData", (
        "Dictionary needs to represent a xData"
    ...
    assert len(names) == 1, (
        "Unknown structure for casting dict to xData"
    ...
    if "name" in elms["header"].keys():
        if elms["header"]["name"] != names[0]:
            raise KeyError(
                "Headers name argument not consistent with name"
        ...
        warnings.warn(
            "Dictionary without 'name' under the 'header' values - creating on the fly"
        )
    ...
    return xData(elms["data"], header=elms["header"], meta=elms["meta"])
...
def pretty_print_get_containers(d, depth=0):
    ...
    if isinstance(d, dict):
        ...
        s = "\n" + "".join(
            [
                pretty_print_get_containers(e, depth=depth)
...
if __name__ == "__main__":
    ...
    tdata.data.append(xData(None, header={"name": "deepest"}))  # type: ignore
